(function(){
	...
	function playGame()
	{
		...
		for(var i = 0; i < aliens.length; i++)
		{
			...
			//Check if the alien had crossed the bottom of the screen
			if(alien.y > canvas.height + alien.height)
			{
				//End game if and alien has reached Earth
				gameState = OVER;
			}
		}
		...
		for(var i = 0; i < aliens.length; i++)
		{
			...
			for(var j = 0; j < missiles.length; j++)
			{
				...
				if(hitTestRectangle(missile, alien) && alien.state === alien.NORMAL)
				{
					...
					//Subtract 1 from the loop counter to compenstate for the removed missile
					j--;
				}
			}
		}
		...
		function makeAlien()
		{
			...
			//Push the alien into both the sprites and alien arrays
			sprites.push(alien);
			aliens.push(alien);
		}
		...
		{
			//Change the alien's state and update the objectToRemove
			alien.state = alien.EXPLODED;
			alien.update();
			
			//Remove the alien after on second
